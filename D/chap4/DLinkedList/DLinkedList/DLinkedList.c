...
void FInsert(List* plist, LData data)
{
	...
	(plist->numofData)++;	// 저장된 노드의 수 하나 증가시킴
}
...
void LInsert(List* plist, LData data)	// 정렬 기준에 따라 함수 실행 달라짐
{
	if (plist->comp == NULL)
		FInsert(plist, data); 
	else
...
int LFirst(List* plist, LData* pdata)
{
	if (plist->head->next == NULL)	// 더미 노드가 NULL을 가리킨다면
		return FALSE;				// 반환 데이터가 없음

	plist->before = plist->head;	// before를 더미 노드를 가리키게 함
